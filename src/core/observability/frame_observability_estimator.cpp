...
FrameObservability FrameObservabilityEstimator::estimate(const FrameInput &frame) const {
  FrameObservability result;
  result.sectors.resize(config_.observability.sector_count);
  if (frame.merged_cloud.empty()) {
  ...
  std::vector<bool> rear_sectors(config_.observability.sector_count, false);
  const float angle_per_sector = 2.0f * kPi / static_cast<float>(config_.observability.sector_count);
  Eigen::Quaternionf inv_orientation = frame.base_pose_in_local.orientation.conjugate().normalized();
