...
bool TraversalCost::publish(const ros::Publisher &publisher,
...
bool TraversalCost::fillMessage(nav_msgs::OccupancyGrid &msg,
                                const std::string &frame_id,
                                const ros::Time &stamp) const {
  const auto params = map_.getTraversalCostParams();
  ...
  try {
    ...
    ROS_WARN("Failed to convert traversal cost layer: %s", e.what());
    return false;
  }
