...
void LidarCoverage::addLidar(const std::string& lidar_name)
{
    ...
    node_->declare_parameter(lidar_name + ".pos_body", std::vector<double>{0.0, 0.0, 0.0});
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "Failed to get position for %s", lidar_name.c_str());
    }

    // ----------- rpy_body -----------
    std::vector<double> rpy_vec;
    node_->declare_parameter(lidar_name + ".rpy_body", std::vector<double>{0.0, 0.0, 0.0});
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "Failed to get euler angle for %s", lidar_name.c_str());
    }
