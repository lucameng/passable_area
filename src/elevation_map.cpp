...
void ElevationMap::processPointCloud(const sensor_msgs::PointCloud2 &ros_cloud,
...
void ElevationMap::cloud2Elevation() {
  ...
  if (bin_width <= 0.0f) {
    ROS_WARN("Invalid bin width %.3f for vertical structure", bin_width);
    return;
  }
  ...
  auto solver_result = ElevationSolver::solve(ctx_solver, solver_params);
  ...
  if (solver_result.ground.size() != expected_cells ||
    ...
    ROS_WARN("ElevationSolver returned mismatched result size. "
