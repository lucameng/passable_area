...
SHLIBS_LOCAL_FILE="${DEB_DIR}/shlibs.local"

# 0) 确保 bloom 已生成 debian/
test -d "${DEB_DIR}" || { echo "debian/ not found. Run bloom-generate first."; exit 1; }

# 1) 写入 shlibs.local：绕过自建日志库依赖检查
cat > "${SHLIBS_LOCAL_FILE}" <<'EOF'
liblog4cplus 2 log4cplus-dev
EOF

# 2) 注入 unit：让 dh_installsystemd 识别并安装
...
# 6) 确保 dh 调用包含 --with systemd（老版本 debhelper 默认不带）
if ! grep -q -- '--with systemd' "${DEB_DIR}/rules"; then
  sed -i 's/^\(\s*dh\s\+\$@\)/\1 --with systemd/' "${DEB_DIR}/rules"
fi

# 7) 确保 .install 包含 service（双保险，即便 dh_installsystemd 未生效也能打包）
