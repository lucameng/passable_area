...
struct FalseObstacleHotspot {
  ...
  float source_clearance = 0.0f;
  float source_support_continuity = 0.0f;
  float source_support_height = 0.0f;
  float source_overhead_height = 0.0f;
  uint8_t source_block_reason = 0U;
  uint8_t source_obstacle_point_publish_status = 0U;
  bool source_low_clearance_bridge_eligible = false;
  std::string source_publish_path;
